...
fetch_key_id() {
    ...
    output=$(simulator -f /app/config/local_centralized_from_compose.toml insecure-key-gen)
    if [[ $? -ne 0 ]]; then
...
fetch_crs_id() {
    ...
    output=$(simulator -f /app/config/local_centralized_from_compose.toml insecure-crs-gen --max-num-bits 2048)
    if [[ $? -ne 0 ]]; then
        echo "Error during key-gen. Exiting."
        exit 1
    fi
